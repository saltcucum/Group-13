...
dat_summary <- anxiety_level %>% 
  ...
  ungroup()
```
#Summary of count, mean and SD of the selected native and non-native samples
```{r}

native_anxiety_level <- filter(anxiety_level, native == 1)
nonnative_anxiety_level <- filter(anxiety_level, native == 2) 

native_summary<- native_anxiety_level %>%
  pivot_longer(anxiety_1:anxiety_5, "Question", values_to = "Response")%>%
  group_by(Question)%>% 
  summarise(count= n(),
            anxiety_mean = mean(Response, na.rm = TRUE),
...
nonnative_summary<- nonnative_anxiety_level %>%
  pivot_longer(anxiety_1:anxiety_5, "Question", values_to = "Response")%>%
  group_by(Question)%>% 
  summarise(count= n(),
            anxiety_mean = mean(Response, na.rm = TRUE),
...
#equal variances - vairances are similar as below
